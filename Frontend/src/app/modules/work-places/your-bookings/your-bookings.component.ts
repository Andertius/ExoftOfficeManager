      ...
      .subscribe(x => {
        ...
        for (let i = 0; i < this.bookingsResponses.length; i++) {
          this.bookings.push({
            ...
            date:
              `${new Date(this.bookingsResponses[i].booking.date).getDate()}` + ' ' +
              `${this.parseMonth(new Date(this.bookingsResponses[i].booking.date).getMonth())}` + ' ' +
              `${new Date(this.bookingsResponses[i].booking.date).getFullYear()}`,
